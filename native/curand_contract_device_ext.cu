...
void philox_add_uniform(torch::Tensor x, torch::Tensor out, unsigned long long seed, double alpha) {
  ...
  philox_add_uniform_kernel<<<blocks_for(out.numel(), threads), threads>>>(
      x.data_ptr<float>(), out.data_ptr<float>(), out.numel(), seed, static_cast<float>(alpha));
}

void philox_threshold(torch::Tensor mask, unsigned long long seed, double p) {
  ...
  philox_threshold_kernel<<<blocks_for(mask.numel(), threads), threads>>>(
      mask.data_ptr<unsigned char>(), mask.numel(), seed, static_cast<float>(p));
}

void philox_dropout(torch::Tensor x, torch::Tensor out, torch::Tensor mask, unsigned long long seed, double p) {
  check_cuda_tensor(x, "x");
  check_cuda_tensor(out, "out");
  check_cuda_tensor(mask, "mask");
  TORCH_CHECK(x.scalar_type() == torch::kFloat32, "x must be float32");
  ...
  philox_dropout_kernel<<<blocks_for(out.numel(), threads), threads>>>(
      ...
      out.numel(), seed, static_cast<float>(p));
}
