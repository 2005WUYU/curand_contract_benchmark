...
@triton.jit
def _lognormal_transform_kernel_32(out_ptr, raw_ptr, n, mean, stddev, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    ...
    ln0 = tl.exp(mean + stddev * n0)
    ln1 = tl.exp(mean + stddev * n1)
    out = tl.join(ln0, ln1)
    tl.store(out_ptr + base, out, mask=mask[:, None])


@triton.jit
def _lognormal_transform_kernel_64(out_ptr, raw_ptr, n, mean, stddev, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    base = (offs * 2)[:, None] + tl.arange(0, 2)[None, :]
...
def generate_lognormal(
    ...
    *,
    ...
    assert_tensor_device_supported(out, op_name="generate_lognormal")
    ...
    if not is_64 and gen_type == GENERATOR_XORWOW:
        generate_xorwow_lognormal_f32(out, generator, mean=mean, stddev=stddev)
        return out
    ...
    if is_64:
        ...
        _lognormal_transform_kernel_64[grid](
        ...
        n_pairs = n // 2
        grid = (triton.cdiv(n_pairs, block_size),)
        _lognormal_transform_kernel_32[grid](
            out.view(-1), raw.view(-1), n_pairs,
            mean, stddev,
            BLOCK=block_size, num_warps=num_warps,
        )
