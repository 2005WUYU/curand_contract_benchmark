...
_SOBOL64_DIRECTION_VECTORS = torch.load(
    str(Path(__file__).parent / "data" / "sobol_dv64.pt"), map_location="cpu"
)

_SOBOL64_MAX_DIMENSIONS = 20000

# Triton GPU kernel (64-bit processed in two halves: low 32 bits + high 32 bits)

@triton.jit
def _sobol64_generate(
...
@triton.jit
def _sobol64_single_dim_kernel(
    out_int32_ptr,
    v0_lo, v1_lo, v2_lo, v3_lo, v4_lo, v5_lo, v6_lo, v7_lo,
    v8_lo, v9_lo, v10_lo, v11_lo, v12_lo, v13_lo, v14_lo, v15_lo,
    v16_lo, v17_lo, v18_lo, v19_lo, v20_lo, v21_lo, v22_lo, v23_lo,
    ...
    result_lo, result_hi = _sobol64_generate(
        v0_lo, v1_lo, v2_lo, v3_lo, v4_lo, v5_lo, v6_lo, v7_lo,
        v8_lo, v9_lo, v10_lo, v11_lo, v12_lo, v13_lo, v14_lo, v15_lo,
        v16_lo, v17_lo, v18_lo, v19_lo, v20_lo, v21_lo, v22_lo, v23_lo,
        v24_lo, v25_lo, v26_lo, v27_lo, v28_lo, v29_lo, v30_lo, v31_lo,
    ...
    # write directly into the int64 out, viewed as int32 pairs (little-endian: lo at 2*i, hi at 2*i+1)
    base = 2 * offs
...
def _launch_sobol64_dim(
    ...
    _sobol64_single_dim_kernel[grid](
        out_int32_dim,
    ...
    def generate_long_long(
        ...
        **kwargs: object,
        ...
        if out.dtype != torch.int64:
            raise TypeError("Sobol64: only uint64 (int64) output is supported.")
        assert_tensor_device_supported(out, op_name="Sobol64.generate_long_long")
        ...
        if dimensions < 1 or dimensions > _SOBOL64_MAX_DIMENSIONS:
            raise ValueError(
                f"Sobol64: dimensions must be between 1 and {_SOBOL64_MAX_DIMENSIONS}, got {dimensions}."
            )
        if n % dimensions != 0:
            raise ValueError(
                f"Sobol64: element count ({n}) must be a multiple of dimensions ({dimensions})."
            )
        if offset_val < 0:
            raise ValueError(f"Sobol64: offset must be >= 0, got {offset_val}.")

        n_points_per_dim = n // dimensions
        dvs = _SOBOL64_DIRECTION_VECTORS[:dimensions]

        # write directly into out, viewed as int32 pairs.
        # int64 little-endian: out_int32[2*i] = lo, out_int32[2*i+1] = hi
        out_int32 = out.view(-1).view(torch.int32)

        for d in range(dimensions):
            # each dim's slice spans 2 * n_points_per_dim int32 elements
            base_int32 = d * (2 * n_points_per_dim)
            out_int32_dim = out_int32[base_int32 : base_int32 + 2 * n_points_per_dim]
            dv64 = dvs[d].tolist()
            dv_lo = [v & 0xFFFFFFFF for v in dv64[:32]]
            dv_hi = [v >> 32 for v in dv64[:32]]
            _launch_sobol64_dim(
                out_int32_dim, dv_lo, dv_hi,
